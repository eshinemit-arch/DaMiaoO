...
- **🧹 智能前言与元数据兼容 (Preamble & Bare Meta)**：为文档而生的 Markdown 往往带有长篇介绍。DaMiaoo 会自动识别首个 `#` (Rank 1) 作为幻灯片主标题，并**智能剥离**在此之前的导言。更强大的是，它现在支持**“裸元数据”**——即使您忘记写 `---` 包裹，只要文件开头是 `author: 张三` 格式，引擎也会精准捕获并将其从正文剔除。
- **✨ 细节语义清洗与章节进化**：告别排版毛刺。支持使用 `\\` 在标题或致谢中执行优雅的“物理换行”；自动抹除多列版式中的冒号间隔符。现在，`@[chapter]` 章节页变得更加聪明：它会自动将章节标题/副标题留作“纯粹过渡页”，而将后续的列表内容自动“顺延”到下一页并套用最合适的响应式布局（Cards/Cols），确保视觉优先级。
- **🎨 UI/UX Pro Max 视觉体系**：内置极致的 `theme-damiaoo.css` 主题。目录页 (`@[toc]`) 现在会自动感应标题缺位并补全“目录”二字，确保全局风格统一。
- **📊 自动化成果审计**：每次构建都会输出报告，监控全案规格、平均密度与版式分布。
...
4. **元数据声明**：引擎现在非常宽容。无论您使用标准的 `---` 严密包裹，还是直接在文件开头书写 `key: value`（裸元数据），引擎都能正确提取属性。如果两者都没有，系统将应用智能算法从正文首行提取标题。
